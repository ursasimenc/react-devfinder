...
function App() {
	...
	useEffect(() => {
		...
		document.querySelector("body")?.classList.add(darkTheme ? "dark" : "light");
	}, []);
	...
	async function getUser(username: any) {
		...
		try {
			...
			setRepos(result.data.public_repos);
			setLocation(result.data.location);
			setTwitter(result.data.twitter_username);
			setCompany(result.data.company);
			setWebsite(result.data.blog);

			result.data.bio === null ? setBio("This profile has no bio") : setBio(result.data.bio);
			result.data.blog === "" ? setWebsite("Not available") : setWebsite(result.data.blog);
			result.data.twitter_username === null ? setTwitter("Not available") : setTwitter(result.data.twitter_username);
			result.data.company === null ? setCompany("Not available") : setCompany(result.data.company);
			result.data.location === null ? setLocation("Not available") : setLocation(result.data.location);
	...
	return (
		<main>
			...
			<div id="search">
				...
				<input type="search" id="search-input" placeholder="Search GitHub username..." onKeyDown={(e) => getUser(e)} />
				<span>{error}</span>
				<button onClick={() => getUser("button")}>Search</button>
			</div>

			<div id="result" className={loading ? "loading" : "loaded"}>
				{!loading && (
					<>
						{" "}
						...
						<div className="profile-info">
							...
							<div className="links">
								<div className={location === "Not available" ? "not-available" : "available"}>
									<div className="icon">
									...
									<div className="links-text">{location}</div>
								</div>
								<div className={twitter === "Not available" ? "not-available" : "available"}>
									<div className="icon">
									...
									<div className="links-text">{twitter}</div>
								</div>
								<div className={website === "Not available" ? "not-available" : "available"}>
									<div className="icon">
								...
								<div className={company === "Not available" ? "not-available" : "available"}>
									...
									<div className="links-text">{company}</div>
								</div>
							</div>
						</div>
					</>
